...
ANALYSIS_MODEL_PURPOSES = frozenset({"identify", "timeline_prepass", "rolling"})
...
def summarize_analysis_cost(
    ...
    pending_jobs = 0
    for job in jobs:
        if str(job.get("purpose") or "") not in ANALYSIS_MODEL_PURPOSES:
            continue
    ...
    return AnalysisCostSummary(
        ...
        complete=(
            pending_jobs == 0
            and normalized["priced_calls"] >= normalized["provider_calls"]
        ),
